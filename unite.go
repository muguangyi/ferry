...
func main() {
	...
	info, err := build.ImportDir(".", 0)
	if nil != err {
		log.Fatal(err)
	}

	fset := token.NewFileSet()
	log.Println("File count: ", len(info.GoFiles))
	for _, file := range info.GoFiles {
		scanFile(fset, file)
	}
}

func scanFile(fset *token.FileSet, file string) []string {
	f, err := parser.ParseFile(fset, file, nil, 0)
	if nil != err {
		log.Fatal(err)
	}

	log.Println("-------------Start ast--------------")
...
func save(target string, data []byte) error {
	return ioutil.WriteFile(strings.ToLower(target)+".proxy.gen.go", data, 0644)
}
