...
func (p *{{$.proxy}}) {{$method.Name}}({{range $j, $param := $method.Params}}{{$param.Name}} {{$param.Type}}{{if le $j $method.PCount}}, {{end}}{{end}}){{if gt $method.RCount 0}}({{range $k, $result := $method.Results}}{{$result.Type}}{{comma $k $method.RCount}}{{end}}){{end}}{
	{{if gt $method.RCount 0}}results, err := p.signaler.CallWithResult("{{$.target}}", "{{$method.Name}}", {{range $j, $param := $method.Params}}{{$param.Name}}{{comma $j $method.PCount}}{{end}})
	{{else}}err := p.signaler.Call("{{$.target}}", "{{$method.Name}}", {{range $j, $param := $method.Params}}{{$param.Name}}{{comma $j $method.PCount}}{{end}})
	{{end}}
	if nil != err {
...
var {{.proxy}}succ bool = seek.Register("{{.target}}", func(signaler seek.ISignaler) interface{} { return &{{.proxy}}{signaler: signaler} })
`
